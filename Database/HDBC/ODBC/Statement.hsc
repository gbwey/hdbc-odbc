...
wrapStmt sstate = Statement
  ...
  , nextResultSet  = fnextResultSet sstate
  }
...
fexecute :: SState -> [SqlValue] -> IO (Either Int [(String, SqlColDesc)])
  ...
  (finish, result) <- withStmtOrDie (sstmt sstate) $ \hStmt -> do
    ...
    case rc of
      0 -> do rowcount <- getSqlRowCount hStmt
              return (True, Left $ fromIntegral rowcount)
      colcount -> do
                     xs <- fgetcolinfo hStmt
                     swapMVar (colinfomv sstate) xs
                     return (False, Right xs)
  -- when finish $ ffinish sstate
  return result

fnextResultSet :: SState -> IO (Maybe (Either Int [(String, SqlColDesc)]))
...
-}
     ...
     case r of
        #{const SQL_SUCCESS} -> do
                                 rc <- getNumResultCols hStmt
                                 hdbcTrace $ "getNumResultCols fnextResultSet: rc=" ++ show rc
                                 case rc of
                                    0 -> do
                                            rowcount <- getSqlRowCount hStmt
                                            return (Just $ Left (fromIntegral rowcount))
                                    colcount -> do
                                                   xs <- fgetcolinfo hStmt
                                                   swapMVar (colinfomv sstate) xs
                                                   return (Just $ Right xs)
